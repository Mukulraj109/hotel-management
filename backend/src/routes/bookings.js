 ...
 */
router.get('/current-hotel', authenticate, ensurePropertyAccess, catchAsync(async (req, res) => {
  res.json({
    status: 'success',
    data: {
 ...
 */
router.get('/upcoming', authenticate, ensurePropertyAccess, catchAsync(async (req, res) => {
  const {
    days = 7,
    page = 1,
    limit = 50
 ...
 */
router.get('/room/:roomId', authenticate, authorize('admin', 'staff', 'frontdesk'), ensurePropertyAccess, catchAsync(async (req, res) => {
...
router.patch('/:id/check-in',
  ...
  catchAsync(async (req, res) => {
    ...
    if (paymentDetails && paymentDetails.paymentMethods && Array.isArray(paymentDetails.paymentMethods)) {
      ...
      console.log('✅ Check-in payment processed:', {
        bookingNumber: updatedBooking.bookingNumber,
        paymentAmount: totalPaymentAmount,
        totalAmount: updatedBooking.totalAmount,
        methods: paymentDetails.paymentMethods.map(pm => `${pm.method}: ₹${pm.amount}`).join(', ')
      });
    }
    ...
    console.log('💰 After save - payment details:', {
      bookingNumber: updatedBooking.bookingNumber,
      totalPaid: updatedBooking.paymentDetails?.totalPaid || 0,
      totalAmount: updatedBooking.totalAmount,
...
router.patch('/:id/check-out',
  ...
  catchAsync(async (req, res) => {
    ...
    console.log('💰 Checkout payment validation:', {
      ...
      hasPaymentDetails: !!booking.paymentDetails,
      bypassBalanceCheck
    });
    ...
    if (settlement.outstandingBalance > 0 || settlement.refundAmount > 0) {
      ...
      console.log('✅ Settlement auto-created at checkout:', {
        bookingNumber: updatedBooking.bookingNumber,
        finalAmount: settlement.finalAmount,
        outstandingBalance: settlement.outstandingBalance,
        refundAmount: settlement.refundAmount,
      ...
      console.log('✅ Settlement auto-completed (fully paid):', {
        bookingNumber: updatedBooking.bookingNumber,
        finalAmount: settlement.finalAmount
      });
    }
...
router.post('/:id/settlement/payment',
  ...
  catchAsync(async (req, res) => {
    ...
    console.log('✅ Settlement payment processed:', {
      ...
      totalPaidWillBeCalculated: 'by pre-save hook',
      previousBalance,
      newBalance: booking.settlementTracking.outstandingBalance,
      methods: paymentMethods.map(pm => `${pm.method}: ₹${pm.amount}`).join(', ')
    });
