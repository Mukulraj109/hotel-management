...
router.get('/metrics', catchAsync(async (req, res) => {
  ...
  // Count rooms per property for debugging
  for (const property of properties) {
    const roomCount = await Room.countDocuments({ hotelId: property._id });
    console.log(`Property "${property.name}" (${property._id}): ${roomCount} rooms`);
  }
