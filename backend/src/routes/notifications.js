...
router.get('/stream', authenticate, (req, res) => {
  ...
  const healthCheck = setInterval(() => {
    ...
    if (timeSinceLastPing > 120000) { // 2 minutes without activity
      logger.debug('[SSE] Connection timeout', { userId });
      cleanup();
    }
    lastPing = Date.now();
  ...
  const notificationHandler = (notification) => {
    try {
      ...
      logger.debug('[SSE] Sent notification', { eventType, userId, notificationType: notification.type });
    } catch (error) {
      logger.error('[SSE] Error sending notification', { userId, error: error.message });
    }
  };
  ...
  const cleanup = () => {
    logger.debug('[SSE] Cleaning up connection', { userId });
    ...
    try {
      ...
      logger.error('[SSE] Error during cleanup', { userId, error: error.message });
    }
  };
  ...
  req.on('error', (error) => {
    logger.error('[SSE] Request error', { userId, error: error.message });
    cleanup();
  });

  res.on('error', (error) => {
    logger.error('[SSE] Response error', { userId, error: error.message });
    cleanup();
  });
  ...
  setTimeout(() => {
    try {
      ...
      logger.error('[SSE] Error sending initial status', { error: error.message });
    }
  }, 1000);

  logger.debug('[SSE] Successfully established connection', { userId });
});
