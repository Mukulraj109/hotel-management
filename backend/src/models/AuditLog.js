...
auditLogSchema.index({ 'sourceDetails.channel': 1, createdAt: -1 });
auditLogSchema.index({ 'reconciliation.status': 1 });
auditLogSchema.index({ 'metadata.batchId': 1 });
...
auditLogSchema.statics.getChangeHistory = function(tableName, recordId, options = {}) {
  const query = { tableName, recordId };
