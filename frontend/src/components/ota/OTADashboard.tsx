...
const OTADashboard: React.FC = () => {
  ...
  // Chart data configurations (Recharts format)
  const bookingTrendChartData = bookingTrends.map(trend => ({
    name: format(new Date(trend.date), 'MMM dd'),
    'Booking.com': trend['booking.com'],
    Expedia: trend['expedia'],
    Airbnb: trend['airbnb'],
    'Hotels.com': trend['hotels.com'],
  }));

  const deviceAnalyticsChartData = deviceAnalytics ? [
    { name: 'Desktop', value: deviceAnalytics.desktop, fill: 'rgba(59, 130, 246, 0.8)' },
    { name: 'Mobile', value: deviceAnalytics.mobile, fill: 'rgba(16, 185, 129, 0.8)' },
    { name: 'Tablet', value: deviceAnalytics.tablet, fill: 'rgba(245, 101, 101, 0.8)' },
  ] : null;

  const revenueByChannelColors = [
    'rgba(59, 130, 246, 0.8)',
    'rgba(16, 185, 129, 0.8)',
  ...
  const revenueByChannelData = otaChannels.map((ch, index) => ({
    name: ch.name,
    revenue: ch.revenue / 1000, // Convert to thousands
    fill: revenueByChannelColors[index % revenueByChannelColors.length],
  }));
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
              <ResponsiveContainer width="100%" height={320}>
                <LineChart data={bookingTrendChartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                  <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                  <YAxis tick={{ fontSize: 11 }} />
                  <RechartsTooltip />
                  <RechartsLegend />
                  <RechartsLine type="monotone" dataKey="Booking.com" stroke="rgb(59, 130, 246)" strokeWidth={2} dot={false} />
                  <RechartsLine type="monotone" dataKey="Expedia" stroke="rgb(16, 185, 129)" strokeWidth={2} dot={false} />
                  <RechartsLine type="monotone" dataKey="Airbnb" stroke="rgb(245, 101, 101)" strokeWidth={2} dot={false} />
                  <RechartsLine type="monotone" dataKey="Hotels.com" stroke="rgb(168, 85, 247)" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </ChartCard>
            ...
            >
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={revenueByChannelData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                  <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                  <YAxis tick={{ fontSize: 11 }} />
                  <RechartsTooltip />
                  <RechartsBar dataKey="revenue" name="Revenue (K)" radius={[4, 4, 0, 0]}>
                ...
                <ResponsiveContainer width="100%" height={270}>
                  <RechartsPieChart>
                    <RechartsPie
                      ...
                      stroke="#fff"
                    >
                      {deviceAnalyticsChartData!.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={entry.fill} />
                      ))}
                    </RechartsPie>
                    <RechartsTooltip />
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant={getChannelStatusColor(channel.status) as unknown}>
                        {channel.status}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-2">
