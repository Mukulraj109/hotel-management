...
const AdminRoomPricing: React.FC = () => {
    ...
    try {
      ...
      if (roomsData.length > 0) {
        ...
        console.log('Available status fields:', {
          status: roomsData[0].status,
          computedStatus: roomsData[0].computedStatus,
          currentStatus: roomsData[0].currentStatus
        });
      }
  ...
  const filteredRooms = Array.isArray(rooms) ? rooms.filter(room => {
    ...
    if (filters.status && filters.status === 'occupied') {
      console.log(`Room ${room.roomNumber}: status=${room.status}, computedStatus=${room.computedStatus}, currentStatus=${room.currentStatus}, using=${roomStatus}`);
    }
